...
describe('Miscellaneous tests', () => {
  ...
  var ASCII_SUPERSETS = THE_ENCODING
    .concat(LEGACY_ENCODINGS)
    .filter(function (e) {
  ...
  it('Supersets of ASCII decode ASCII correctly', () => {
  ...
  it('Non-fatal errors at EOF', () => {
    assert_throws(TypeError, function () { new TextDecoder('utf-8', { fatal: true }).decode(new Uint8Array([0xff])); });
    // This should not hang:
    new TextDecoder('utf-8').decode(new Uint8Array([0xff]));

    assert_throws(TypeError, function () { new TextDecoder('utf-16le', { fatal: true }).decode(new Uint8Array([0x00])); });
    // This should not hang:
    new TextDecoder('utf-16le').decode(new Uint8Array([0x00]));

    assert_throws(TypeError, function () { new TextDecoder('utf-16be', { fatal: true }).decode(new Uint8Array([0x00])); });
    // This should not hang:
  ...
  it('Replacement encoding labels', () => {
    [
      'csiso2022kr',
      'hz-gb-2312',
      'iso-2022-cn',
  ...
  it('ArrayBuffer, ArrayBufferView and buffer offsets', () => {
  ...
  it('NONSTANDARD - regression tests', () => {
    assert_array_equals(
  ...
  it('NONSTANDARD - GB 18030 2000 vs 2005: U+1E3F, U+E7C7 (encoding)', () => {
    // Regression test for https://github.com/whatwg/encoding/issues/22
    assert_array_equals(
  ...
  it('NONSTANDARD - iso-2022-jp encoding attack (encoding)', () => {
  ...
  ['utf-16le', 'utf-16be'].forEach(function (encoding) {
    it('NONSTANDARD - ' + encoding + ' (encoding)', () => {
